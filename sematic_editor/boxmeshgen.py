...
class Panel:
    ...
    def __init__(self, panel, panelName, mesh_resolution):
        ...
        self.glob_offset = -1
        self.orig_pivot = panel.get('_orig_pivot', None)
...
class BoxMesh(wrappers.VisPattern):
    ...
    def load(self):
        ...
        # Apply surface stitches (e.g. patch pocket attachments)
    ...
    def _local_to_global_vertex_id(self, panel, loc_id):
        n_stitches_panel = panel.n_stitches
        if loc_id < n_stitches_panel:
            return self.verts_loc_glob[(panel.panel_name, loc_id)]
        return loc_id + panel.glob_offset - n_stitches_panel

    def _surface_stitch_target_loc_ids(self, panel, segment, count, max_distance=None):
        if count <= 0:
            return []
    ...
    def _surface_stitch_pairs(self):
        ...
        for stitch_id, stitch in enumerate(surface_stitches):
            try:
                ...
                source_panel = self.panels[source["panel"]]
                target_panel = self.panels[target["panel"]]
                source_edge = source_panel.edges[int(source["edge"])]
            ...
            if target.get("coordinate_space") == "target_panel_2d":
                pivot = target_panel.orig_pivot
                if pivot is not None:
                    pdx, pdy = float(pivot[0]), float(pivot[1])
                    target_segment = [
            ...
            for source_id, target_id in zip(source_loc_ids, target_loc_ids):
                pairs.append(
                    (
                        self._local_to_global_vertex_id(source_panel, source_id),
                        self._local_to_global_vertex_id(target_panel, target_id),
    ...
    def _remap_surface_stitch_vertices(self, pairs):
        ...
        while len(self.verts_glob_loc) < len(self.vertices):
            self.verts_glob_loc.append([])
        while len(self.stitch_segmentation) < len(self.vertices):
            self.stitch_segmentation.append([])

        for glob_id in range(len(self.vertices)):
            root = find(glob_id)
            if root not in old_to_new:
                old_to_new[root] = new_idx
                new_vertices.append(self.vertices[root])
                new_verts_glob_loc.append(list(self.verts_glob_loc[root]))
                new_stitch_segmentation.append(list(self.stitch_segmentation[root]))
                new_idx += 1
            else:
                target_new = old_to_new[root]
                new_verts_glob_loc[target_new].extend(self.verts_glob_loc[glob_id])
        ...
        for face in self.faces:
            ...
            if new_face[0] == new_face[1] or new_face[1] == new_face[2] or new_face[0] == new_face[2]:
                continue
        ...
        for tex_face in self.faces_with_texture:
            ...
            new_tex_face[0] = old_to_new.get(tex_face[0], tex_face[0])
            new_tex_face[2] = old_to_new.get(tex_face[2], tex_face[2])
            new_tex_face[4] = old_to_new.get(tex_face[4], tex_face[4])
            if new_tex_face[0] == new_tex_face[2] or new_tex_face[2] == new_tex_face[4] or new_tex_face[0] == new_tex_face[4]:
                continue
    ...
    def _add_surface_stitch_orig_lens(self, old_to_new):
        for (v1, v2), length in list(self.orig_lens.items()):
            new_v1 = old_to_new.get(v1, v1)
            new_v2 = old_to_new.get(v2, v2)
            if new_v1 != v1 or new_v2 != v2:
                new_key = (min(new_v1, new_v2), max(new_v1, new_v2))
                if new_key not in self.orig_lens:
                    self.orig_lens[new_key] = length
    ...
    def apply_surface_stitches(self):
        ...
        print(
            ...
            f"{vertex_count_before - len(self.vertices)} mesh vertices"
        )

    def _fill_missing_orig_lens(self):
        """Add orig_lens entries for faces that were missed during finalise_mesh
        (e.g. pocket panel faces with no stitch vertices)."""
        for face in self.faces:
            v0, v1, v2 = face
            for a, b in [(v0, v1), (v1, v2), (v0, v2)]:
                key = (min(a, b), max(a, b))
                if key not in self.orig_lens:
                    dist = float(np.linalg.norm(
                        np.array(self.vertices[b]) - np.array(self.vertices[a])
                    ))
                    self.orig_lens[key] = dist
